...
def make_instance(argv = (), usage = False, verbose = False):
  ...
  if usage:
    print('Arguments: [key1=value1 [key2=value2 ...]]')
    return

  # initialize the list of lines to write to the instance file.
  out = []

  # write a header, if requested.
  if verbose:
    out.append('')
    out.append('/* Generated by build.py with arguments:')
    out.append(' *  "{}"'.format(' '.join(argv)))
    out.append(' */')

  # add a few preprocessor statements.
  out.append('')
  ...
  for param in params:
    ...
    try:
      ...
      print('/* Invalid value "{}" for {} */'.format(value, param))
      return
